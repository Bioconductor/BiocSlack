...
sm_latest <- sm %>%
  ...
  mutate(
    ...
    latest = as_datetime(as.integer(ts_formatted)),
    # Latest thread reply message
    latest_reply = as_datetime(as.integer(root$latest_reply)), 
  ...
  ungroup() %>%
  mutate(latest_date = as_date(final_latest))
```
...
all <- left_join(sc, sm_latest, by = "id") |>
  ...
  rename(Name = name.x, Topic = topic_value, Description = purpose_value, `Number of Members` = num_members, `Last Activity` = latest)
```
