...
sm_latest <- sm |>
  ...
  mutate(
    ...
    # Calculate time difference in minutes for relative time
    TimeDiff = difftime(now("UTC"), final_latest, units = "mins"),
    # Combine both precise and relative times in a single string
...
all |>
  reactable(
    columns = list(
      ...
      `Last Activity` = colDef(html = TRUE, defaultSortOrder = "desc") # Enable HTML rendering for Last Activity
    ),
    outlined = TRUE,
    bordered = TRUE,
    resizable = TRUE,
